...
function RecentNotesFullSection({
  contactId,
  onOpenNoteComposer,
}: {
  contactId: number;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        console.warn("파이프라인3 최근 활동노트 불러오기 실패", error);
        if (alive) setNotes([]);
      } finally {
        if (alive) setLoading(false);
      }
  ...
  function formatNoteDate(value: string | null) {
    ...
    const normalized = raw.length <= 10 ? `${raw.slice(0, 10)}T00:00:00` : raw;
  ...
  return (
    <section className="premium-card p-5">
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="crm-tiny">
              Quick actions 하단에서 고객의 전체 활동노트를 바로 확인합니다
            </p>
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <span
            className="rounded-full border px-3 py-1 text-[12px] font-[760]"
            style={{
          ...
          >
            총 {notes.length.toLocaleString()}건
          </span>
          <button
            type="button"
            onClick={onOpenNoteComposer}
      ...
      {loading ? (
        ...
        <div
          className="flex min-h-[220px] items-center justify-center rounded-[18px] border border-dashed text-[13px] font-semibold"
          style={{ color: "var(--text-faint)", borderColor: "var(--border)" }}
        >
          등록된 활동노트가 없습니다.
        </div>
          ...
          {notes.map((note) => (
            ...
            >
              <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                <div className="flex min-w-0 items-center gap-2">
                  <span
                    className="rounded-full px-2.5 py-1 text-[12px] font-[800]"
                    style={{
                  ...
                  {note.author ? (
                    <span
                      className="rounded-full px-2.5 py-1 text-[12px] font-[720]"
                      style={{
                        background: "var(--surface)",
              ...
              <p
                className="whitespace-pre-wrap text-[13px] font-[640] leading-7"
                style={{ color: "var(--text-muted)" }}
              >
                {note.content || "-"}
              </p>
