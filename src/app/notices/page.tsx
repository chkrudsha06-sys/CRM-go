  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm">
      <div className="flex h-[96vh] w-full max-w-2xl flex-col overflow-hidden rounded-[20px]"
        ...
        <div className="min-h-0 flex flex-1 flex-col gap-4 overflow-y-auto px-6 py-5">
          <div>
          ...
          <div>
            <p className="crm-tiny mb-2 font-normal">파일 첨부</p>
            <label
            ...
            >
              ...
              <p className="text-[13px] font-normal" style={{ color:"var(--text-muted)" }}>파일 첨부 (선택)</p>
              <p className="crm-tiny font-normal">클릭하거나 드래그하여 파일을 업로드하세요</p>
            </label>
            {/* 기존 첨부파일 (수정 모드) */}
            {existing.length > 0 && (
              <div className="mt-2 space-y-1">
                {existing.map((e,i)=>(
                  <div key={`ex-${i}`} className="flex items-center gap-2 rounded-[8px] px-3 py-2"
                    ...
                    <span className="flex-1 truncate text-[12px] font-normal" style={{ color:"var(--text)" }}>{e.name}</span>
              ...
              <div className="mt-2 space-y-1">
                {files.map((f,i)=>(
                  <div key={`new-${i}`} className="flex items-center gap-2 rounded-[8px] px-3 py-2"
                    style={{ background:"var(--surface-2)", border:"1px solid var(--border)" }}>
                    <File size={13} style={{ color:"var(--accent-text)" }} />
                    <span className="flex-1 truncate text-[12px] font-normal" style={{ color:"var(--text)" }}>{f.name}</span>
          ...
          <div className="flex min-h-[260px] flex-1 flex-col">
            <p className="crm-tiny mb-1 font-normal">내용 *</p>
            <textarea className="min-h-[260px] w-full flex-1 resize-none rounded-[8px] border px-3 py-3 text-[13px] font-normal leading-relaxed outline-none"
