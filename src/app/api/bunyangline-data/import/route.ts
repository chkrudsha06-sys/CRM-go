...
function sectionPriority(value: unknown) {
  return SECTION_PRIORITY[normalizeAdSection(value)] || 0;
}

function isFeaturedSection(value: unknown) {
  return sectionPriority(value) > SECTION_PRIORITY['일반구인글'];
}

function phoneKey(value: unknown) {
  const phone = normalizePhone(value);
  if (!phone || phone === '-' || phone.length < 8) return null;
  return phone.replace(/\D/g, '') || null;
}

function comparableDateValue(row: { posted_datetime?: string | null; posted_at?: string | null; created_at?: string | null; source_url?: string | null }) {
  return row.posted_datetime || row.posted_at || row.created_at || '';
}

function compareBySectionPriority<T extends { ad_section?: string | null; posted_datetime?: string | null; posted_at?: string | null; created_at?: string | null; source_url?: string | null }>(a: T, b: T) {
  const sectionDiff = sectionPriority(b.ad_section) - sectionPriority(a.ad_section);
  if (sectionDiff !== 0) return sectionDiff;

  const dateDiff = comparableDateValue(b).localeCompare(comparableDateValue(a));
  if (dateDiff !== 0) return dateDiff;

  return String(a.source_url || '').localeCompare(String(b.source_url || ''));
}

function filterGeneralPhoneDuplicates<T extends { source_url: string | null; manager_phone?: string | null; ad_section?: string | null; posted_datetime?: string | null; posted_at?: string | null; created_at?: string | null }>(
  rows: T[],
  existingRows: ExistingPhoneRow[] = []
) {
  const featuredPhones = new Set<string>();
  const existingGeneralSourcesByPhone = new Map<string, Set<string>>();

  for (const existing of existingRows) {
    const phone = phoneKey(existing.manager_phone);
    if (!phone) continue;

    if (isFeaturedSection(existing.ad_section)) {
      featuredPhones.add(phone);
      continue;
    }

    if (!existingGeneralSourcesByPhone.has(phone)) existingGeneralSourcesByPhone.set(phone, new Set());
    if (existing.source_url) existingGeneralSourcesByPhone.get(phone)?.add(existing.source_url);
  }

  const sortedRows = [...rows].sort(compareBySectionPriority);
  const generalPhones = new Set<string>();
  const keptRows: T[] = [];
  let skippedGeneralDuplicateCount = 0;

  for (const row of sortedRows) {
    const phone = phoneKey(row.manager_phone);

    if (isFeaturedSection(row.ad_section)) {
      keptRows.push(row);
    ...
    const existingGeneralSources = existingGeneralSourcesByPhone.get(phone);
    const hasDifferentExistingGeneral =
      existingGeneralSources && Array.from(existingGeneralSources).some((sourceUrl) => sourceUrl !== row.source_url);

    if (featuredPhones.has(phone) || generalPhones.has(phone) || hasDifferentExistingGeneral) {
      skippedGeneralDuplicateCount += 1;
      continue;
    }
    ...
    if (!existingGeneralSourcesByPhone.has(phone)) existingGeneralSourcesByPhone.set(phone, new Set());
    if (row.source_url) existingGeneralSourcesByPhone.get(phone)?.add(row.source_url);
  }

  return {
    rows: keptRows.sort(compareBySectionPriority),
    skippedGeneralDuplicateCount,
  };
}
