...
function compareBySectionPriority(a: any, b: any) {
...
function filterGeneralPhoneDuplicates(rows: any[]) {
  ...
  for (const row of sortedRows) {
    ...
    if (featuredPhones.has(phone) || generalPhones.has(phone)) continue;

    keptRows.push(row);
    generalPhones.add(phone);
  }

  return keptRows.sort(compareBySectionPriority);
}
...
function normalizeRows(rows: any[]) {
  ...
  const visibleRows = filterGeneralPhoneDuplicates(fixedRows);

  const phoneCounts = visibleRows.reduce<Record<string, number>>((acc, row) => {
