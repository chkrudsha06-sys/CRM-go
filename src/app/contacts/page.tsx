...
function getCurrentCrmUserInfo() {
  try {
    ...
    return {
      isAdmin,
      isExec,
      name: isExec ? normalizeAssignedTo(rawName) : rawName,
    };
  } catch {
    return { isAdmin: false, isExec: false, name: "" };
  }
...
export default function ContactsPage() {
  ...
  const [currentUserInfo] = useState<{ isAdmin: boolean; isExec: boolean; name: string }>(() => getCurrentCrmUserInfo());
  ...
  useEffect(() => {
    ...
    const loadRecords = async () => {
      try {
        ...
        const currentUser = getCurrentCrmUserInfo();
        const execName = currentUser.isExec && !currentUser.isAdmin ? currentUser.name : null;
  ...
  const resetFilters = () => {
    ...
    setFilterStage("");
    setFilterGrade("");
    if (currentUserInfo.isAdmin) setFilterAssigned("전체");
  };
