...
function sectionRank(value: string) {
  return SECTION_RANK[value] || 0;
}

function isFeaturedSection(value: string) {
  return sectionRank(value) > SECTION_RANK['일반구인글'];
}
...
function comparableDateValue(item: BunyanglineItem) {
  return item.posted_datetime || item.posted_at || item.crawled_at || '';
}

function compareBySectionPriority(a: BunyanglineItem, b: BunyanglineItem) {
  const sectionDiff = sectionRank(b.ad_section) - sectionRank(a.ad_section);
...
function filterGeneralPhoneDuplicates(items: BunyanglineItem[]) {
  const sortedItems = [...items].sort(compareBySectionPriority);
  const featuredPhones = new Set<string>();
  const generalPhones = new Set<string>();
  const keptItems: BunyanglineItem[] = [];
  const skippedGeneralDuplicates: BunyanglineItem[] = [];

  for (const item of sortedItems) {
    const phone = phoneKey(item.manager_phone);
